      ...
      <div className="w-full max-w-sm mx-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} className="input-field" placeholder="ivan@example.com" />
          </div>
          <div>
            <label className="text-xs font-bold uppercase tracking-wider text-gray-400 block mb-1">Пароль</label>
            <div className="relative">
              <input type={show ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} className="input-field pr-10" placeholder="••••••••" />
        ...
        <p className="text-center text-sm text-gray-400 mt-6">
          Нет аккаунта? <Link href="/auth/register" className="text-ice-red font-bold hover:underline">Зарегистрироваться</Link>
        </p>
      </div>
    </div>
  )
